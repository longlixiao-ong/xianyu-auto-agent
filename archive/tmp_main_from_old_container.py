# =============================================================================
# 归档文件 — 不再参与运行
# 原始路径: tmp_main_from_old_container.py
# 归档原因: 旧容器环境的 main 入口备份，已被 src/main.py 取代
# 保留仅供历史参考，不包含 admin_server 集成、policy_engine 等新模块
# =============================================================================
import base64
import json
import asyncio
...
from XianyuApis import XianyuApis, RiskControlError
import sys
import random
...
class XianyuLive:
    def __init__(self, cookies_str):
        ...
        self.selling_items_snapshot_on_start = os.getenv(
            ...
            os.getenv("SELLING_ITEMS_SNAPSHOT_ON_START", "False"),
        ).lower() == "true"
        self.selling_items_snapshot_path = os.getenv(
            "MY_ITEMS_SNAPSHOT_PATH",
            os.getenv("SELLING_ITEMS_SNAPSHOT_PATH", os.path.join("data", "my_items.json")),
        )
    ...
    def refresh_selling_items_snapshot(self):
        try:
            ...
            payload = self.xianyu.get_my_items(
                ...
                headless=os.getenv("SELLING_ITEMS_BROWSER_HEADLESS", "True").lower() == "true",
            )
    ...
    def extract_image_urls(self, payload):
        """从消息结构中递归提取图片URL。"""
        urls = []
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            decision = bot.decide_reply_action(
            ...
            # 有图优先走多模态回复
            if image_urls:
                observation = bot.observe_images(
                    send_message,
                    item_description,
                    context=context,
                    image_urls=image_urls,
                ...
                bot_reply = bot.generate_reply_with_images(
                    send_message,
                    item_description,
                    context=context,
                    image_urls=image_urls,
    ...
    async def main(self):
        while True:
            try:
                ...
                wait_seconds = int(os.getenv("RISK_CONTROL_RETRY_INTERVAL", "300"))
