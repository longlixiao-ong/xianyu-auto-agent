...
def account_id_from_cookies(cookies: Dict[str, str]) -> str:
    """从已解析的 Cookie 字典取出账号 ID (unb)。缺失或为空时抛出 ValueError（明确错误，避免裸 KeyError）。"""
    uid = str(cookies.get("unb") or "").strip()
    if not uid:
        raise ValueError(
            "Cookie 中缺少必填字段 unb（账号 ID）。请从闲鱼网页复制完整 Cookie，"
            "确认包含 unb=…（通常为数字账号）。"
        )
    return uid
...
def generate_sign(t: str, token: str, data: str) -> str:
    """生成签名"""
    msg = f"{token}&{t}&{MTOP_APP_KEY}&{data}"
